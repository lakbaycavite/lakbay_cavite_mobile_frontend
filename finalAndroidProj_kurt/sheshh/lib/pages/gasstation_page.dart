...
class _GasStationsPageState extends State<GasStationsPage> {
  ...
  final List<gmaps.Marker> _gasStationMarkers = [];
  ...
  Future<void> _getCurrentLocation() async {
    ...
    setState(() {
      ...
      _gasStationMarkers.add(_userLocationMarker!);
    });
  ...
  Future<void> _getGasStations({int radius = 5000}) async {
    setState(() {
      _selectedRadius = radius;
    });

    final url = Uri.parse(
      'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${_currentLocation.latitude},${_currentLocation.longitude}&radius=$radius&type=gas_station&key=$googleMapsApiKey',
    );

    final response = await http.get(url);
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      List<dynamic> gasStations = jsonData['results'];

      setState(() {
        _gasStationMarkers.clear();
        _gasStationMarkers.add(_userLocationMarker!);
        _gasStations = gasStations; // Update the list of gas stations

        for (var gasStation in gasStations) {
          final gasStationLatLng = gmaps.LatLng(
            gasStation['geometry']['location']['lat'],
            gasStation['geometry']['location']['lng'],
          );

          _gasStationMarkers.add(
            gmaps.Marker(
              markerId: gmaps.MarkerId(gasStation['place_id']),
              position: gasStationLatLng,
              infoWindow: gmaps.InfoWindow(
                title: gasStation['name'],
                snippet: gasStation['vicinity'],
                onTap: () {
                  _showGasStationDetailsDialog(gasStation, gasStationLatLng);
                },
              ),
            ),
          );
  ...
  Future<void> _startNavigation(dynamic gasStation) async {
    final gasStationLatLng = gmaps.LatLng(
      gasStation['geometry']['location']['lat'],
      gasStation['geometry']['location']['lng'],
    );
    ...
    final url = Uri.parse(
      'google.navigation:q=${gasStationLatLng.latitude},${gasStationLatLng.longitude}',
    );
  ...
  Future<void> _drawPolylineToGasStation(gmaps.LatLng destination) async {
    setState(() {
      _polylines.clear();
    });
  ...
  void _showGasStationDetailsDialog(dynamic hospital, gmaps.LatLng gasStationLatLng) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          backgroundColor: Color(0xFF035594),
          title: Text(hospital['name'], style: const TextStyle(
            fontSize: 20,
            fontFamily: 'Poppins',
          ...
          content: Column(
            ...
            children: [
              Text('Address: ${hospital['vicinity']}', style: const TextStyle(
                fontSize: 14,
                fontFamily: 'Poppins',
                color: Colors.white,
              ),),
              Text('Rating: ${hospital['rating'] ?? 'No rating'}', style: const TextStyle(
                fontSize: 13,
                fontFamily: 'Poppins',
                color: Colors.white,
              ),),
          ...
          actions: [
            TextButton(
              ...
              onPressed: () async {
                ...
                await _drawPolylineToGasStation(gasStationLatLng);
              },
            ),
            TextButton(
  ...
  void _centerOnUserLocation() {
    _mapController.animateCamera(
      gmaps.CameraUpdate.newLatLngZoom(_currentLocation, 15),
    );
  }
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Row(
          children: [
            Icon(
              Icons.local_gas_station, // Use the gas icon from Material Icons
              color: Colors.white,
              size: 30,
            ),
      ...
      body: Stack(
        children: [
         GoogleMap(
            ...
            markers: _showMarkers ? _gasStationMarkers.toSet() : {},
          ...
          // Gas Station List - floating at the top
          Positioned(
            top: 0,
            left: 0,
            right: 0,
